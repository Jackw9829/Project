...
try {
    ...
    $stmt = $pdo->prepare("
        ...
        WHERE q.quiz_id = ? AND q.teacher_id = ?
    ");
    $stmt->execute([$quiz_id, $_SESSION['user_id']]);
    $quiz = $stmt->fetch();

    if (!$quiz) {
        throw new Exception('Quiz not found or unauthorized access');
    }
    ...
    $stmt = $pdo->prepare("
        SELECT 
            ...
            SUM(CASE WHEN sa.answer = q.correct_answer THEN 1 ELSE 0 END) as correct_answers,
            COUNT(DISTINCT q.question_id) as total_questions,
            TIMESTAMPDIFF(MINUTE, s.started_at, s.submitted_at) as time_taken
        FROM submissions s
        JOIN users u ON s.student_id = u.user_id
        LEFT JOIN student_answers sa ON s.submission_id = sa.submission_id
        JOIN questions q ON sa.question_id = q.question_id
        WHERE s.quiz_id = ?
        GROUP BY s.submission_id, s.submitted_at, u.name
...
<html>
<head>
    <title>MathQuest - Teacher Grading</title>
    ...
    <style>
        * {
        ...
        /* Style for correct/incorrect marks */
        .grade-table td:nth-child(n+4) {
            font-size: 1.2rem;
            color: #333;
        }
...
<body>
    ...
    <main class="grade-container">
        ...
        <?php else: ?>
            <h1><?php echo htmlspecialchars($quiz['title']); ?> - Submissions</h1>
                    ...
                    <?php else: ?>
                        <?php foreach ($submissions as $submission): ?>
                            <tr>
                                ...
                                <td>
                                    <?php 
                                        $score = ($submission['total_questions'] > 0) 
                                            ? ($submission['correct_answers'] / $submission['total_questions']) * 100 
                                            : 0;
                                        echo number_format($score, 1) . '%';
                                    ?>
                                </td>
                                <td><?php echo $submission['time_taken'] . ' mins'; ?></td>
                                <td><?php echo date('M j, Y g:i A', strtotime($submission['submitted_at'])); ?></td>
                                <td>
                                    <a href="review_submission.php?submission_id=<?php echo $submission['submission_id']; ?>" 
