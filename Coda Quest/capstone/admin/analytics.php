...
function getQuizCompletionData() {
    try {
        ...
        // Validate the result
        if (!is_array($result)) {
            error_log("Quiz completion data query returned non-array result");
            return [];
        }
        
        // Add debugging
        error_log("Quiz completion data: " . print_r($result, true));
        
        return $result;
    } catch (Exception $e) {
        error_log("Error fetching quiz completion data: " . $e->getMessage());
        return [];
    }
}
...
function getUserActivityData($days = 30) {
    try {
        ...
        // Validate the result
        if (!is_array($result)) {
            error_log("User activity data query returned non-array result");
            return [];
        }
        ...
        // Add debugging
        error_log("User activity data: " . print_r($filledData, true));
        
        return $filledData;
    } catch (Exception $e) {
        error_log("Error fetching user activity data: " . $e->getMessage());
        return [];
    }
}
...
$sql = "SELECT username FROM students LIMIT 2";
